        ...
        if len(self.calls) == 1:
            return OcrExtraction(
                lines=[
                    ...
                    OcrLine(text="어쉬밀크클릿 [ 1,600", confidence=0.62, line_id=2, page_order=2),
                ],
                raw_tokens=[],
                quality_score=0.82,
                low_quality_reasons=[],
        ...
        if len(self.calls) == 1:
            return OcrExtraction(
                lines=[
                    ...
                    OcrLine(text="2023/11/24 1층", confidence=0.98, line_id=1, page_order=1),
                    OcrLine(text="어쉬밀크클릿 [ 1,600", confidence=0.62, line_id=2, page_order=2),
                    OcrLine(text="이에 2 4,000", confidence=0.76, line_id=3, page_order=3),
...
def test_receipt_service_applies_qwen_item_normalization_and_revalidates(tmp_path: Path) -> None:
    ...
    assert qwen.payloads
    assert parsed["items"][0]["raw_name"] == "어쉬밀크클릿 ["
