...
| `OIP (20).webp` | 0.3333 | grocery partial receipt. 6-digit PLU code 제거, embedded barcode noise tail 정리, 일부 exact alias로 일부 회복됐지만 `청양고추`/`생목심` 축은 아직 OCR 정규화가 약함 |
...
- 이번 기준에는 [OIP (20).webp](C:/Users/USER-PC/Desktop/jp/.worktrees/codex-hwpx-proposal-patch/output/제비/OIP%20(20).webp)도 grocery partial acceptance gold로 편입했다.
  ...
  - 이후 `6-digit PLU code` 제거, embedded barcode noise tail 정리, `사각햇번300g -> 사각햇반300g`, `깐양과 -> 깐양파` exact alias를 넣어 `item_f1 = 0.3333`까지 회복했다.
...
- 이번 보강의 핵심:
  ...
  - `OIP (20).webp`: grocery partial receipt에 `6-digit PLU code` 제거와 embedded barcode noise tail cleanup을 넣어 raw item 정리가 일부 회복
...
- 현재 실사 gold 14장 기준 review 축도 정렬됐다.
  ...
  - 현재 최약군은 여전히 `OIP (20).webp (0.3333)`이고, 다음은 `OIP (9).webp (0.6316)`, `OIP (7).webp (0.8571)`, `OIP (8).webp (0.8571)`이다.
