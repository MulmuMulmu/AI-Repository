...
- `purchased_at`: 구매일
- `food_items`: 식품 후보 목록
- `food_items[].product_name`: OCR/파싱된 상품명
...
A. 아니다. 백엔드 담당자가 재료 DB와 사용자 수정 흐름을 기준으로 처리한다. AI는 `POST /ai/ingredient/match`를 공개 API로 제공하지 않는다.
...
A. OCR 보조와 회귀 테스트용 내부 로직이다. 외부 계약은 아니며, 백엔드가 호출할 API로 보지 않는다.
...
A. 추천 컨테이너는 `POST /recommend`만 공개한다.
...
A. 기본 조건은 `coverageRatio >= 0.5`다. 즉 레시피 재료를 모두 가지고 있거나, 최소 절반 이상 가지고 있어야 추천 후보가 된다.

Q. 거리/벡터 기반 추천은 실제로 어떻게 동작하는가?

A. 사용자 보유 재료와 각 레시피의 필요 재료를 같은 재료 ID 공간의 벡터로 본다. 예를 들어 사용자가 `양파`, `계란`, `밥`을 가지고 있고 어떤 레시피가 `계란`, `밥`, `간장`, `참기름`을 필요로 하면, 겹치는 재료와 부족한 재료를 계산한 뒤 두 벡터가 얼마나 가까운지 점수화한다.

처리 순서는 아래와 같다.

1. 레시피마다 필요한 재료 ID 목록을 만든다.
2. 사용자 보유 재료 ID와 레시피 재료 ID의 교집합을 구한다.
3. `coverageRatio = 보유 중인 레시피 재료 수 / 레시피 전체 재료 수`를 계산한다.
4. `coverageRatio`가 기본 기준인 `0.5`보다 낮으면 후보에서 제외한다.
5. 남은 후보에 대해 가중치 기반 벡터 점수와 cosine similarity를 계산한다.
6. 알레르기, 비선호 재료, 제외 카테고리, 제외 키워드가 있으면 hard filter로 제거한다.
7. 선호 재료, 선호 카테고리, 선호 키워드는 soft boost로 점수에 더한다.
8. 최종 `score`가 높은 순서로 정렬하고 `topK`만 반환한다.

Q. 레시피 재료 벡터의 가중치는 어떻게 잡는가?

A. 모든 재료를 똑같이 보지 않는다. 현재 엔진은 재료별 중요도를 아래 요소로 가중한다.

- `IDF`: 모든 레시피에 흔한 재료보다 특정 레시피를 더 잘 설명하는 재료를 더 중요하게 본다.
- `CATEGORY_IMPORTANCE`: 정육/계란, 해산물, 쌀/면/빵처럼 요리 중심성이 높은 재료군은 높게 보고, 소스/조미료/오일은 낮게 본다.
- `amount multiplier`: 레시피에 필요한 양이 큰 재료는 더 중요하게 본다.

즉 소금, 간장처럼 자주 나오는 조미료 하나를 가지고 있다고 높은 점수를 주지 않고, 고기/계란/밥/면처럼 레시피의 중심이 되는 재료를 가지고 있을 때 더 강하게 반영한다.

Q. cosine similarity는 여기서 어떤 의미인가?

A. 두 벡터의 방향이 얼마나 비슷한지 보는 값이다. 사용자 보유 재료 벡터가 레시피 재료 벡터와 같은 방향에 가까울수록 값이 커진다. 단순히 몇 개가 겹치는지만 보는 것이 아니라, 어떤 중요도의 재료가 겹치는지도 반영한다.

예를 들어 두 레시피가 모두 2개 재료가 겹치더라도, 하나는 핵심 재료인 `돼지고기`, `양파`가 겹치고 다른 하나는 `간장`, `소금`만 겹치면 전자가 더 가까운 레시피로 평가된다.

Q. 핵심 재료는 어떻게 판단하는가?

A. 각 레시피에서 가중치가 높은 재료를 핵심 재료로 본다. 레시피 재료 수가 적으면 핵심 재료 1개, 중간 규모면 2개, 많으면 3개를 잡는다. 이 핵심 재료를 얼마나 보유했는지가 `coreCoverage`에 반영된다.
...
```text
score =
  0.4  * weightedCoverage
+ 0.2  * cosineSimilarity
+ 0.2  * coreCoverage
+ 0.2  * coverageRatio
+ preferenceBonus
- 0.1  * missingPenalty
- 0.15 * coreMissingPenalty
```

점수는 마지막에 `0.0`에서 `1.0` 사이로 제한한다. 이 구조 때문에 “재료를 절반 이상 가지고 있는가”로 후보를 먼저 좁히고, 그 안에서 “핵심 재료를 얼마나 가지고 있는가”, “가중 벡터가 얼마나 가까운가”, “부족 재료가 얼마나 중요한가”로 순위를 정한다.
